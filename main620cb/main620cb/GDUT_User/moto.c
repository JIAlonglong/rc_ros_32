...
void m3508_update_m3508_info(CAN_RxHeaderTypeDef *msg,uint8_t	can1_RxData[8])
{
    ...
    case M3508_CHASSIS_MOTOR_ID_1:
		...
		{ 
			MOTOR_REAL_INFO[6].ANGLE= (can1_RxData[0] << 8) | can1_RxData[1];  // 转子机械角度
			MOTOR_REAL_INFO[6].RPM     = (can1_RxData[2] << 8) | can1_RxData[3];  // 实际转子转速
			MOTOR_REAL_INFO[6].CURRENT = (can1_RxData[4] << 8) | can1_RxData[5];  // 实际转矩电流

		}; break;

		case M2006_CHASSIS_MOTOR_ID_1://2006电机（左边）
		{ 
			MOTOR_REAL_INFO[7].ANGLE= (can1_RxData[0] << 8) | can1_RxData[1];  // 转子机械角度
			MOTOR_REAL_INFO[7].RPM     = (can1_RxData[2] << 8) | can1_RxData[3];  // 实际转子转速
			MOTOR_REAL_INFO[7].CURRENT = (can1_RxData[4] << 8) | can1_RxData[5];  // 实际转矩电流
        ...
        PID_incremental_PID_calculation(&MOTOR_PID_RPM[i], MOTOR_REAL_INFO[i].RPM, MOTOR_REAL_INFO[i].TARGET_RPM);//速度环
	      ...
	      HomingMode(&MOTOR_REAL_INFO[i]);
			...
			case POSITION_TORQUE_MODE://位置转矩模式
			  ...
			  PID_incremental_PID_calculation(&MOTOR_PID_RPM[i], MOTOR_REAL_INFO[i].RPM, MOTOR_PID_POS[i].output);//速度环
				PID_position_PID_calculation(&MOTOR_PID_POS[i], MOTOR_REAL_INFO[i].REAL_ANGLE, MOTOR_REAL_INFO[i].pos_torquemode.Pos);//位置环
			  MOTOR_PID_RPM[i].output = MaxMinLimit(MOTOR_PID_RPM[i].output,MOTOR_REAL_INFO[i].pos_torquemode.TARGET_TORQUE);//限制转矩模式时电流值
			  //判断是否夹取
			  //flag = 1夹取成功
			...
			case SPEED_TARQUE_CONTROL_MODE://速度转矩模式
				PID_incremental_PID_calculation(&MOTOR_PID_RPM[i], MOTOR_REAL_INFO[i].RPM, MOTOR_REAL_INFO[i].vel_torquemode.Vel);//速度环
				MOTOR_PID_RPM[i].output = MaxMinLimit(MOTOR_PID_RPM[i].output,MOTOR_REAL_INFO[i].vel_torquemode.TARGET_TORQUE);//限制转矩模式时电流值
			  //判断是否夹取
			  //flag = 1夹取成功
			  if(fabsf(MOTOR_REAL_INFO[i].RPM) <=10){		//2
...
//					MOTOR_REAL_INFO[i].TARGET_RPM = 0;
	...
	for(int i = 0; i < 8; i++)
			{
				if(MOTOR_REAL_INFO[i].unitMode == CURRENT_MODE){//电流模式下的特殊情况
			}
			else{
